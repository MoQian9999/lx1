...
// ---------- 悔棋状态 ----------
let lastCaptured = null;        // 最后一步被吃的棋子（悔棋时还原）
let prevMoveFromRow = -1;
let prevMoveFromCol = -1;
let prevMoveToRow = -1;
let prevMoveToCol = -1;
...
function fitCanvasToViewport() {
  ...
  const cellByW = Math.floor((maxWidth - 60) / (COLS - 1));
  const cellByH = Math.floor((maxHeight - 60) / (ROWS - 1));
  cellSize = Math.min(MAX_CELL_SIZE, cellByW, cellByH);
  cellSize = Math.max(MIN_CELL_SIZE, cellSize);
  padding = Math.max(16, Math.floor(cellSize * 0.5));
  pieceRadius = Math.floor(cellSize * 0.42);
  hintRadius = Math.max(4, Math.floor(cellSize * 0.13));
  canvas.width = (COLS - 1) * cellSize + padding * 2;
  canvas.height = (ROWS - 1) * cellSize + padding * 2;
  drawBoard();
}
...
function intersectionToPixel(row, col) {
  ...
  return {
    x: padding + d.col * cellSize,
    y: padding + d.row * cellSize,
  };
}

function pixelToIntersection(mx, my) {
  const dc = Math.round((mx - padding) / cellSize);
  const dr = Math.round((my - padding) / cellSize);
...
function drawBoard(skipAnimating) {
  ...
  for (let r = 0; r < ROWS; r++) {
    ...
    ctx.lineTo(padding + (COLS - 1) * cellSize, y);
    ctx.stroke();
  }
  ...
  for (let c = 0; c < COLS; c++) {
    ...
    if (c === 0 || c === COLS - 1) {
      ...
      ctx.lineTo(x, padding + (ROWS - 1) * cellSize);
      ctx.stroke();
    } else {
      // 内线断开（上半部分：行0-4）
      ctx.beginPath();
  ...
  const leftX = padding + 1.5 * cellSize;
  const rightX = padding + 6.5 * cellSize;
...
function drawPiece(row, col, piece, scaleX) {
  ...
  // 棋子圆形
  ctx.beginPath();
  ctx.arc(0, 0, pieceRadius, 0, Math.PI * 2);

  if (piece.color === "red") {
    const grad = ctx.createRadialGradient(-4, -4, 2, 0, 0, pieceRadius);
  ...
  // 棋子内圈
  ctx.beginPath();
  ctx.arc(0, 0, pieceRadius - 4, 0, Math.PI * 2);
...
function executeMove(fromRow, fromCol, toRow, toCol, onDone) {
  ...
  // 将当前 last 转移到 prev（支持悔棋还原）
  prevMoveFromRow = lastMoveFromRow;
  prevMoveFromCol = lastMoveFromCol;
  prevMoveToRow = lastMoveToRow;
  prevMoveToCol = lastMoveToCol;
...
function onCanvasClick(e) {
  ...
  const { row, col } = pixelToIntersection(pt.x, pt.y);
...
function requestUndo() {
  if (gameOver || pendingUndoRequest || lastMoveFromRow < 0) return;
  if (myTurn && prevMoveFromRow < 0) return;
...
function performUndo(undoTwoStones) {
  if (undoTwoStones) {
    ...
    if (lastMoveFromRow >= 0) {
      board[lastMoveFromRow][lastMoveFromCol] = board[lastMoveToRow][lastMoveToCol];
      board[lastMoveToRow][lastMoveToCol] = lastCaptured;
    }
    if (prevMoveFromRow >= 0) {
      board[prevMoveFromRow][prevMoveFromCol] = board[prevMoveToRow][prevMoveToCol];
      board[prevMoveToRow][prevMoveToCol] = prevCaptured;
    }
    lastMoveFromRow = -1;
    lastMoveFromCol = -1;
    lastMoveToRow = -1;
    ...
    lastMoveFromRow = prevMoveFromRow;
    lastMoveFromCol = prevMoveFromCol;
    lastMoveToRow = prevMoveToRow;
    lastMoveToCol = prevMoveToCol;
    lastCaptured = prevCaptured;
    prevMoveFromRow = -1;
    prevMoveFromCol = -1;
    prevMoveToRow = -1;
    prevMoveToCol = -1;
